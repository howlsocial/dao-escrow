...
# user 2, juno1kwzwzeytzp7sc0qt6uk7dr0uvvcln5qsn3wa9c
echo "major dove rabbit train novel debate cheese exist surround comic drop burger" | $BINARY keys add another-user --recover --keyring-backend test
...
$BINARY tx wasm instantiate $CONTRACT_CODE "$INIT" --from "validator" --label "dao escrow contract" $TXFLAG --no-admin 
RES=$?
